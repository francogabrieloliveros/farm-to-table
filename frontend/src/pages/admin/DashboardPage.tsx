      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-6">
        ...
        <StatCard
          ...
          value={
            isLoading
              ? "..."
              : (stats?.totalPendingOrders ?? 0).toLocaleString()
          }
        ...
        <StatCard
          ...
          value={
            isLoading ? "..." : (stats?.totalProducts ?? 0).toLocaleString()
          }
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 bg-white border border-[#E8E7E4] rounded-3xl shadow-[0_8px_30px_rgb(0,0,0,0.04)] overflow-hidden">
          <div className="px-8 py-6 border-b border-[#F4F3F1] flex items-center justify-between">
            <h2 className="text-lg font-bold text-[#1C4419] manrope">
              Recent Transactions
            </h2>
            <Link to="/admin/orders">
              <button className="text-xs font-bold text-[#1C4419] hover:underline uppercase tracking-wider">
                View All
              </button>
            </Link>
          </div>
                  ...
                  stats.recentOrders.map((row) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <td className="px-4 py-5">
                          <span
                            className={`text-[10px] px-2.5 py-1 rounded-lg font-black border uppercase tracking-wider ${statusStyles[statusLabel]}`}
                          >
                            {statusLabel}
                          </span>
                        </td>
                        <td className="px-8 py-5 text-right font-black text-[#1C4419]">
  ...
  return (
    <div className="bg-white border border-[#E8E7E4] rounded-3xl p-6 shadow-[0_8px_30px_rgb(0,0,0,0.02)] hover:shadow-[0_8px_30px_rgb(0,0,0,0.06)] transition-all duration-300 relative overflow-hidden group">
      <div className="flex justify-between items-start relative z-10">
        ...
        <div
          className={`flex items-center gap-0.5 text-[10px] font-black px-2 py-1 rounded-lg ${isPositive ? "bg-emerald-50 text-emerald-600" : "bg-rose-50 text-rose-600"}`}
        >
          {isPositive ? (
      ...
      <div className="mt-6 relative z-10">
        <p className="text-[10px] font-black text-[#9CA3AF] uppercase tracking-widest mb-1">
          {label}
        </p>
        <p className="text-2xl font-black text-[#1C4419] tracking-tight manrope">
          {value}
        </p>
      </div>
      <div
        className={`absolute -right-4 -bottom-4 w-24 h-24 rounded-full opacity-[0.03] group-hover:scale-150 transition-transform duration-700 ${colors[color].split(" ")[0]}`}
      />
    </div>
  );
}
