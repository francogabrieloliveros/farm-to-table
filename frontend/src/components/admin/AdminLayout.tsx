...
export default function AdminLayout({ children }: { children: ReactElement }) {
  ...
  const navItems = [
    ...
    { id: "users", label: "Citizens", icon: <Users size={20} />, path: "/admin/users" },
    ...
    { id: "reports", label: "Analytics", icon: <BarChart2 size={20} />, path: "/admin/reports" },
  ];
        ...
        <div className="h-20 flex items-center px-6 border-b border-[#F4F3F1]">
          {!isCollapsed && (
            <div className="flex flex-col">
              <span className="text-xl font-black text-[#1C4419] tracking-tight manrope">Farm-to-Table</span>
              <span className="text-[10px] font-bold text-[#7E2700] uppercase tracking-widest -mt-1">Farm-to-Table Admin</span>
            </div>
          )}
          {isCollapsed && <div className="w-8 h-8 bg-[#1C4419] rounded-lg flex items-center justify-center text-white font-black text-xs">FTT</div>}
        </div>
          ...
          {navItems.map((item) => {
            ...
            return (
              <Link key={item.id} to={item.path}>
                ...
                >
                  <span className={`${isActive ? "text-white" : "text-[#6B7280] group-hover:text-[#1C4419]"}`}>
                    {item.icon}
                  </span>
                  {!isCollapsed && <span>{item.label}</span>}
                  {isActive && !isCollapsed && <div className="ml-auto w-1.5 h-1.5 rounded-full bg-white/40" />}
                </button>
              </Link>
            );
          })}
          ...
          {!isCollapsed && (
            <div className="flex items-center gap-3 px-2 py-3 mb-2">
              ...
              <div className="flex flex-col truncate">
                <span className="text-sm font-bold text-[#1C4419] truncate">{user?.firstName} {user?.lastName}</span>
                <span className="text-[10px] text-[#6B7280] truncate">{user?.email}</span>
              </div>
            </div>
          )}
